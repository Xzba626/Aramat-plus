...
import { operationTypeSchema } from "@/lib/validators";
...
export async function GET() {
  try {
    ...
    const items = await prisma.operationType.findMany({
...
export async function POST(req: Request) {
  try {
    ...
    const denied = requireOwnerOrManager(user);
    if (denied) return denied;
    const body = operationTypeSchema.parse(await req.json());
    const item = await prisma.operationType.create({
      data: { name: body.name, code: body.code, companyId: user!.companyId },
    });
